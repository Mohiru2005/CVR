...
export default function Home() {
  ...
  // Always reset active tab to 'dashboard' upon login
  useEffect(() => {
    if (user) {
  ...
  const handleRestoreFromSnapshot = async (fileName: string) => {
    if (confirm(`Are you sure you want to restore data from "${fileName}"? Any missing rows will be recovered back to the database.`)) {
      setIsRestoring(true);
      try {
        const res = await fetch('/api/backup', {
          method: 'PUT',
  ...
  const handleAdminDirectDelete = async (paymentId: string) => {
    if (!user?.isAdmin) return;
    if (confirm('Admin Action: Permanently delete this payment row from the database?')) {
      try {
        const token = localStorage.getItem('cvr_auth_token') || user?.token;
  ...
  const handleUserRequestDelete = async (payment: any) => {
    if (confirm(`Send a deletion request to Master Admin for payment of ₹${payment.amount} (${payment.targetBank})?`)) {
      try {
        const res = await fetch('/api/delete-requests', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        const data = await res.json();
        if (data.success) {
          alert('Deletion request submitted! Master Admin has been notified.');
          fetchPayments();
        } else {
  ...
  const handleProcessDeleteRequest = async (requestId: string, action: 'ACCEPT' | 'REJECT') => {
    ...
    const confirmMsg = action === 'ACCEPT'
      ? 'Accept this deletion request and permanently delete this payment?'
      : 'Reject this deletion request and keep the payment row?';
  ...
  const handleDeleteUser = async (userToDelete: { id: string; name: string }) => {
    if (!user?.isAdmin) return;
    if (confirm(`Are you sure you want to remove user "${userToDelete.name}"?`)) {
      try {
        const token = localStorage.getItem('cvr_auth_token') || user?.token;
  ...
  const handleExportExcel = () => {
    if (payments.length === 0) {
      alert('No payment rows to export with current filters.');
      return;
    }
  ...
  if (!mounted || isLoading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#f8fafc]">
        <div className="w-12 h-12 rounded-2xl bg-slate-900 flex items-center justify-center text-white shadow-xl mb-3">
          <span className="font-black text-sm tracking-wider">CVR</span>
        </div>
        <div className="flex items-center gap-2 text-slate-400 text-xs font-semibold uppercase tracking-wider">
          <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 brand-dot-pulse" />
          <span>CVR Agencies</span>
        </div>
      </div>
    );
  }
      ...
      {user ? (
        ...
        <div className="min-h-screen flex">
          ...
          <div className="flex-1 lg:pl-72 flex flex-col min-h-screen">
            ...
            <header className="sticky top-0 z-30 bg-white/90 backdrop-blur-md border-b border-slate-200/80 px-4 sm:px-6 h-16 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2.5">
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-white bg-emerald-600 hover:bg-emerald-700 rounded-xl transition-all shadow-xs cursor-pointer active:scale-95"
                >
                  <PlusCircle size={14} />
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-slate-600 hover:text-red-600 bg-white hover:bg-red-50 border border-slate-200 hover:border-red-200 rounded-xl transition-all shadow-2xs cursor-pointer active:scale-95"
                  title="Sign Out"
                >
                  <LogOut size={14} />
            ...
            <main className="flex-1 p-4 sm:p-6 md:p-8 max-w-6xl w-full mx-auto animate-fade-in">
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 rounded-2xl p-6 sm:p-8 text-white shadow-xl relative overflow-hidden">
                    ...
                    <div className="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      <div>
                        <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 mb-2">
                          <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 brand-dot-pulse" />
                          <span>{user.isAdmin ? 'MASTER ADMIN SESSION' : 'USER SESSION'}</span>
                        </div>
                        ...
                        <p className="text-xs sm:text-sm text-slate-300 mt-1 max-w-xl">
                          CVR Agencies Pvt. Ltd. • Payment & Collection Portal.
                        </p>
                      </div>

                      <button
                        onClick={() => setIsAddPaymentOpen(true)}
                        className="px-5 py-3.5 bg-emerald-500 hover:bg-emerald-400 text-slate-900 font-black text-sm rounded-xl shadow-lg transition-all flex items-center justify-center gap-2 cursor-pointer active:scale-95 shrink-0"
                      >
                        <PlusCircle size={18} />
                        <span>+ Add Money</span>
                      </button>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5">
                    <div className="p-5 bg-white rounded-2xl border border-slate-200 shadow-card-subtle">
                      ...
                      <div className="text-[11px] text-slate-400 mt-1">Across all recorded banks</div>
                    </div>
                    ...
                    <div className="p-5 bg-white rounded-2xl border border-slate-200 shadow-card-subtle">
                      <div className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-1 flex items-center justify-between">
                        <span>Excel Sheet Rows</span>
                        <FileSpreadsheet size={15} className="text-emerald-600" />
                      </div>
                      <div className="text-2xl sm:text-3xl font-black text-slate-900">
                        {paymentStats.totalCount} Entries
                      </div>
                  ...
                  <div className="bg-white rounded-2xl border border-slate-200 p-5 sm:p-6 shadow-card-subtle">
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        <h2 className="text-base font-black text-slate-900">Recent Payment Entries</h2>
                        <p className="text-xs text-slate-500">Live transactions recorded across all accounts</p>
                      </div>
                      <button
                        onClick={() => setActiveTab('data')}
                    ...
                    <div className="divide-y divide-slate-100">
                      {payments.slice(0, 5).map((p) => (
                        <div key={p.id} className="py-3 flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <div className="text-sm font-bold text-slate-900">{p.senderName}</div>
                              <div className="text-xs text-slate-400 flex items-center gap-2">
                                <span>📅 {formatDateDisplay(p.date)}</span>
                                <span>•</span>
                                <span className="text-slate-600 font-medium">🏦 {p.targetBank}</span>
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="text-right">
                            ...
                            <div className="text-[10px] text-slate-400">By: {p.createdByName}</div>
                          </div>
                        </div>
                      ))}
                      ...
                      {payments.length === 0 && (
                        <div className="py-8 text-center text-xs text-slate-400">
                          No payments recorded yet. Click "+ Add Money" to add the first entry!
                        </div>
                      )}
                    </div>
                  </div>
              ...
              {activeTab === 'data' && (
                <div className="space-y-4">
                  ...
                  <div className="bg-white rounded-2xl border border-slate-200 p-4 sm:p-5 shadow-card-subtle">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500">
                            Showing {payments.length} row{payments.length !== 1 ? 's' : ''} • Total: ₹{paymentStats.totalAmount.toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <button
                          ...
                          className="px-3.5 py-2 bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs rounded-xl shadow-xs transition-all flex items-center gap-1.5 cursor-pointer active:scale-95"
                          title="Download filtered rows as .xlsx"
                        >
                        ...
                        <button
                          ...
                          title="Refresh Table"
                        >
                          <RefreshCw size={15} className={loadingPayments ? 'animate-spin' : ''} />
                        </button>
                      </div>
                    </div>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        {isFilterActive && (
                          <button
                            ...
                            className="text-xs font-bold text-red-600 hover:text-red-700 flex items-center gap-1 cursor-pointer bg-red-50 hover:bg-red-100 px-2.5 py-1 rounded-lg transition-colors"
                          >
                            <RotateCcw size={12} />
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2.5">
                        ...
                        <div>
                          ...
                          <div className="relative">
                            ...
                            <input
                              ...
                              className="w-full pl-8 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500"
                            />
                          </div>
                        </div>
                        ...
                        <div>
                          ...
                          <input
                            ...
                            className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold text-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500"
                          />
                        </div>
                        ...
                        <div>
                          ...
                          <select
                            ...
                            className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold text-slate-700 focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500 cursor-pointer"
                          >
                  ...
                  <div className="bg-white rounded-2xl border border-slate-300 shadow-card-subtle overflow-hidden">
                      ...
                      <table className="w-full text-left border-collapse text-xs sm:text-sm">
                        ...
                        <thead>
                          <tr className="bg-slate-900 text-white font-bold tracking-wider uppercase text-[11px] border-b border-slate-800">
                            <th className="py-3 px-3.5 w-14 text-center border-r border-slate-800">#</th>
                            <th className="py-3 px-4 border-r border-slate-800">Date (DD/MM/YYYY)</th>
                            <th className="py-3 px-4 border-r border-slate-800">Sender / User</th>
                            <th className="py-3 px-4 text-right border-r border-slate-800">Amount (₹)</th>
                            <th className="py-3 px-4 border-r border-slate-800">Bank to be Delivered</th>
                            <th className="py-3 px-3 text-center w-36">Delete Control</th>
                          </tr>
                        </thead>
                        ...
                        <tbody className="divide-y divide-slate-200">
                          {payments.map((p, index) => {
                            ...
                            return (
                              ...
                              >
                                {/* Row Number */}
                                <td className="py-3 px-3.5 text-center font-mono font-bold text-slate-400 border-r border-slate-200 bg-slate-100/60">
                                  {index + 1}
                                </td>

                                {/* Date (DD/MM/YYYY) */}
                                <td className="py-3 px-4 font-semibold text-slate-800 border-r border-slate-200 whitespace-nowrap">
                                ...
                                <td className="py-3 px-4 text-right font-mono font-black text-emerald-700 border-r border-slate-200 whitespace-nowrap">
                                  ₹{p.amount.toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                                </td>

                                {/* Target Bank */}
                                <td className="py-3 px-4 font-medium text-slate-700 border-r border-slate-200 whitespace-nowrap">
                                  <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-slate-100 text-slate-800 border border-slate-200 font-semibold text-xs">
                                ...
                                <td className="py-3 px-3 text-center whitespace-nowrap">
                                  {user.isAdmin ? (
                                    ...
                                    <button
                                      ...
                                      className="px-2.5 py-1 text-xs font-bold text-red-600 bg-red-50 hover:bg-red-100 border border-red-200 rounded-lg transition-colors cursor-pointer flex items-center justify-center gap-1 mx-auto"
                                    ...
                                    <span className="inline-flex items-center gap-1 px-2.5 py-1 text-[11px] font-bold text-amber-800 bg-amber-100 border border-amber-300 rounded-lg">
                                    ...
                                    <button
                                      ...
                                      className="px-2.5 py-1 text-[11px] font-bold text-amber-700 bg-amber-50 hover:bg-amber-100 border border-amber-200 rounded-lg transition-colors cursor-pointer flex items-center justify-center gap-1 mx-auto"
                          ...
                          {payments.length === 0 && !loadingPayments && (
                            <tr>
                              <td colSpan={6} className="py-12 text-center text-slate-400 text-xs">
                                <FileSpreadsheet size={32} className="mx-auto mb-2 opacity-40 text-emerald-600" />
                                <p className="font-bold text-slate-700 text-sm mb-1">No payment entries matching filter</p>
                                <p className="max-w-xs mx-auto mb-3">Try adjusting your date range, bank, or search name.</p>
                                {isFilterActive && (
                                  <button
                                    onClick={handleResetFilters}
                        ...
                        {payments.length > 0 && (
                          <tfoot>
                            <tr className="bg-slate-100 font-bold border-t-2 border-slate-300 text-slate-900 text-xs sm:text-sm">
                              <td className="py-3 px-3.5 text-center border-r border-slate-200 font-mono">
                                Σ
                              </td>
                              <td colSpan={2} className="py-3 px-4 border-r border-slate-200 uppercase tracking-wider text-[11px] font-black text-slate-700">
                                Total Collection ({payments.length} Rows)
                              </td>
                              <td className="py-3 px-4 text-right font-mono font-black text-emerald-700 text-sm sm:text-base border-r border-slate-200">
                                ₹{paymentStats.totalAmount.toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                              </td>
              ...
              {activeTab === 'recovery' && (
                <div className="space-y-6">
                  ...
                  <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-card-subtle">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      <div className="flex items-start gap-3.5">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            <h2 className="text-lg font-black text-slate-900">Data Recovery & Automatic Backups</h2>
                            <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-emerald-100 text-emerald-800 border border-emerald-300">
                              Active Sync
                            </span>
                          </div>
                          <p className="text-xs text-slate-500 mt-1 max-w-xl">
                            The system automatically archives 10-day & weekly Excel snapshots to your device folder every time you open or use the application.
                          </p>
                          <div className="mt-2 text-[11px] text-slate-400 font-mono flex items-center gap-1.5">
                            ...
                            <span>Saved Location: <span className="text-slate-700 font-semibold">{backupFolder || 'd:\\work\\CVR\\backups\\'}</span></span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <button
                          ...
                          className="px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 disabled:bg-slate-300 text-white font-bold text-xs rounded-xl shadow-xs transition-all flex items-center gap-1.5 cursor-pointer active:scale-95"
                        >
                          <Sparkles size={15} />
                          <span>{loadingBackups ? 'Saving...' : 'Create Instant Backup'}</span>
                        </button>
                  ...
                  <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-card-subtle">
                    ...
                    <div className="divide-y divide-slate-100">
                      {backupsList.map((b) => (
                        <div key={b.fileName} className="py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 hover:bg-slate-50/70 p-3 rounded-xl transition-colors">
                          ...
                          <div className="flex items-center gap-2 shrink-0 self-end sm:self-center">
                            ...
                            <button
                              ...
                              className="px-3.5 py-2 bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border border-emerald-200 font-bold text-xs rounded-xl transition-all flex items-center gap-1.5 cursor-pointer active:scale-95"
                      ...
                      {backupsList.length === 0 && !loadingBackups && (
                        <div className="py-12 text-center text-slate-400 text-xs">
                          ...
                          <p className="max-w-xs mx-auto mb-3">Click "Create Instant Backup" to generate your first 10-day snapshot.</p>
                          <button
                            onClick={handleCreateInstantBackup}
                            className="px-4 py-2 bg-slate-900 text-white font-bold text-xs rounded-xl"
                          >
              ...
              {activeTab === 'requests' && user.isAdmin && (
                <div className="space-y-6">
                  <div className="bg-white rounded-2xl border border-amber-200 shadow-card-subtle p-6">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-2.5">
                    ...
                    <div className="divide-y divide-slate-100">
                      {deleteRequests.map((req) => (
                        <div key={req.id} className="py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-amber-50/40 p-4 rounded-2xl border border-amber-100 my-2">
                          <div className="space-y-1.5">
                            <div className="flex items-center gap-2 text-xs font-bold text-slate-800">
                              <span className="text-amber-800 bg-amber-100 px-2.5 py-0.5 rounded text-[11px] uppercase font-black">
                            ...
                            <div className="text-base font-black text-slate-900 flex flex-wrap items-center gap-x-2.5 gap-y-1">
                              ...
                              <span className="text-xs font-medium text-slate-700 bg-white px-2.5 py-1 rounded-lg border border-slate-200">
                          ...
                          <div className="flex items-center gap-2 shrink-0">
                            <button
                              ...
                              className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white font-bold text-xs rounded-xl shadow-xs transition-all flex items-center gap-1.5 cursor-pointer active:scale-95"
                            ...
                            <button
                              ...
                              className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs rounded-xl transition-all flex items-center gap-1 cursor-pointer"
              ...
              {activeTab === 'users' && user.isAdmin && (
                <div className="space-y-6">
                  <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-card-subtle">
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        <h2 className="text-base font-black text-slate-900">Database User Accounts</h2>
                        <p className="text-xs text-slate-500 mt-0.5">
                          Confidential directory • Accessible only by Master Admin.
                        </p>
                      </div>
                      <div className="flex items-center gap-2">
                        <button
                        ...
                        <div className="text-xs font-bold bg-slate-100 text-slate-700 px-3 py-1 rounded-full">
                    ...
                    <div className="divide-y divide-slate-100">
                      ...
                      <div className="py-3.5 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <div className="w-9 h-9 rounded-xl bg-slate-900 text-emerald-400 flex items-center justify-center font-bold text-xs">
                            <ShieldCheck size={18} />
                          </div>
                          <div>
                            <div className="text-sm font-bold text-slate-900 flex items-center gap-2">
                ...
                <div className="space-y-6">
                  <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-card-subtle">
                    ...
                    <p className="text-xs text-slate-500 mt-1 mb-6">
                      Account & profile settings for CVR Agencies Pvt. Ltd.
                    </p>
                    ...
                    <div className="space-y-3">
                      <div className="p-3.5 bg-slate-50 rounded-xl border border-slate-100 flex items-center justify-between text-xs">
                        ...
                        <span className="font-bold text-slate-900">CVR Agencies Pvt. Ltd.</span>
                      </div>
                      <div className="p-3.5 bg-slate-50 rounded-xl border border-slate-100 flex items-center justify-between text-xs">
                        <span className="font-semibold text-slate-700">Logged User</span>
                        <span className="font-bold text-slate-900">{user.name}</span>
                      </div>
                      <div className="p-3.5 bg-slate-50 rounded-xl border border-slate-100 flex items-center justify-between text-xs">
                        <span className="font-semibold text-slate-700">Account Type</span>
                        <span className="font-bold text-emerald-600">{user.isAdmin ? 'Master Administrator' : 'Standard User'}</span>
                      </div>
                      <div className="p-3.5 bg-slate-50 rounded-xl border border-slate-100 flex items-center justify-between text-xs">
                        <span className="font-semibold text-slate-700">Data Protection Status</span>
                        <span className="font-bold text-emerald-600">Auto 10-Day Excel Sync Active</span>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <footer className="py-4 text-center text-slate-400 text-xs font-medium border-t border-slate-100 bg-white/50">
        ...
        <main className="min-h-screen flex flex-col items-center justify-center p-4 sm:p-6 w-full">
          ...
          />
          <footer className="py-3 text-center text-slate-400 text-xs font-medium">
